...
type StartsWith<T extends readonly unknown[], U extends readonly unknown[]> = T extends [infer TFirst, ...infer TRest]
    ? U extends [infer UFirst, ...infer URest]
        ? Equal<TFirst, UFirst> extends true
    ...
    : U extends []
        ? true
        : false;
type ContainsAsSubarray<T extends readonly unknown[], U extends readonly unknown[]> = StartsWith<T, U> extends true
    ? true
    : T extends [infer First, ...infer Rest]
        ? ContainsAsSubarray<Rest, U>
        : false;

declare const TagKeySymbol: unique symbol;
type TagKey = typeof TagKeySymbol;

type Universe = { [x: string]: never };
type RemoveUniverse<T> = T extends Universe & infer U
    ? U
    : never;

type GetTags<X> = Equal<X ,any> extends true
    ? []
    : TagKey extends keyof X
        ? [keyof Omit<RemoveUniverse<X[TagKey]>, TagKey>, ...GetTags<RemoveUniverse<X[TagKey]>>]
        : [];

type TagInner<X, T extends string> = TagKey extends keyof X
    ? Omit<X, TagKey> & { [x in TagKey]?: Universe & TagInner<RemoveUniverse<X[TagKey]>, T> }
    : X & { [x in TagKey]?: Universe & { [x in T]?: unknown }};
type Tag<X, T extends string> = [Equal<X, any>, Equal<X, never>] extends [false, false]
    ? true extends Equal<X, null> | Equal<X, undefined>
        ? X
        : TagInner<X, T>
    : Tag<0, T>;

type UnTag<X> = X extends null | undefined ? X : Omit<X, TagKey>;

type HasTag<X, T extends string> = T extends GetTags<X>[number] ? true : false;
type HasTags<X, T extends readonly string[]> = ContainsAsSubarray<GetTags<X>, T>;
type HasExactTags<X, T extends readonly string[]> = Equal<GetTags<X>, T>;
