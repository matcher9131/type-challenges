...
> = A extends readonly []
    ...
    : B extends readonly []
        ? A
        : [A, B] extends [readonly [(infer AFirst)?, ...infer ARest], readonly [(infer BFirst)?, ...infer BRest]]
            ? "0" extends keyof A | keyof B // returns true if one of them is a tuple
                ? [[], []] extends [A, B] // returns true if both A and B are tuples with all the elements optional or arrays
                    ? [FlattenObject<AFirst & BFirst>?, ...IntersectParameters<ARest, BRest>]
                    : [FlattenObject<AFirst & BFirst>, ...IntersectParameters<ARest, BRest>]
                : FlattenObject<AFirst & BFirst>[]
            : never;
