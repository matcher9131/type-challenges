...
type IsLessThan<X extends number, Y extends number, C extends unknown[] = []> = C["length"] extends Y
    ? false
    : C["length"] extends X
        ? true
        : IsLessThan<X, Y, [...C, unknown]>;

type InclusiveRangeInner<L extends number, H extends number, C extends unknown[] = [], Result extends number[] = []> = C["length"] extends H
    ? [...Result, H]
    : C["length"] extends L
        ? C["length"] extends H
            ? [L]
            : InclusiveRangeInner<L, H, [...C, unknown], [L]>
        : Result extends []
            ? InclusiveRangeInner<L, H, [...C, unknown], []>
            : InclusiveRangeInner<L, H, [...C, unknown], [...Result, C["length"]]>;


type InclusiveRange<Lower extends number, Higher extends number> = IsLessThan<Higher, Lower> extends true
    ? []
    : InclusiveRangeInner<Lower, Higher>;
