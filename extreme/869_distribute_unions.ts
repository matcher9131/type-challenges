...
type DistributeTuple<T extends unknown[]> = T extends [infer First, ...infer Rest]
    ? First extends First
        ? DistributeUnions<First> extends infer DistributedFirst
            ? DistributedFirst extends DistributedFirst
                ? [DistributedFirst, ...DistributeTuple<Rest>]
                : never
            : never
        : never
    : [];

type DistributeObject<T extends object, K extends keyof T = keyof T> = [K] extends [never]
    ? {}
    : K extends K
        ? DistributeUnions<T[K]> extends infer V
            ? V extends V
                ? { [k in K & string]: V } & DistributeObject<Omit<T, K>>
                : never
            : never
        : never;
...
type DistributeUnions<T> = T extends unknown[]
    ? DistributeTuple<T>
    : T extends object
        ? FlattenObject<DistributeObject<T>>
        : T;
