...
type Contains<T, U> = T extends [infer First, ...infer Rest]
    ? Equal<First, U> extends true
        ? true
        : Contains<Rest, U>
    : false;
type Unique<T> = T extends [...infer Rest, infer Last]
    ? [...Unique<Rest>, Contains<Rest, Last> extends true ? never : Last]
    : [];

function uniqueItems<const T>(items: T extends Unique<T> ? T : Unique<T>): Unique<T> {
    return items;
}
