...
type KeyToPath<K extends string | number, IsArray extends boolean> = (IsArray extends true
    ? `[${K}]` | `.[${K}]`
    : never
) | `.${K}`;

type ObjectKeyPathsInner<T, K extends keyof T = keyof T> = K extends string | number
    ? `${KeyToPath<K, T extends unknown[] ? true : false>}${(T[K] extends object ? ObjectKeyPathsInner<T[K]> : "") | ""}`
    : never;

type RemoveInitialDot<S extends string> = S extends `.${infer T}`
    ? T
    : S;

type ObjectKeyPaths<T extends object> = RemoveInitialDot<ObjectKeyPathsInner<T>>;
