...
type Assign<T extends Record<string, unknown>, U extends unknown[]> = U extends [infer First, ...infer Rest]
    ? First extends Record<string, unknown>
        ? Assign<{
            [P in keyof T | keyof First]: P extends keyof First ? First[P] : P extends keyof T ? T[P] : never;
        }, Rest>
        : Assign<T, Rest>
    : T;
