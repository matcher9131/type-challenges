...
type ParsePrintFormatInner<S extends string, Pre extends boolean = false> = S extends `${infer First}${infer Rest}`
    ? Pre extends true
        ? First extends keyof ControlsMap
            ? [ControlsMap[First], ...ParsePrintFormatInner<Rest, false>]
            : ParsePrintFormatInner<Rest, false>
        : First extends "%"
            ? ParsePrintFormatInner<Rest, true>
            : ParsePrintFormatInner<Rest, false>
    : []
type ParsePrintFormat<S extends string> = ParsePrintFormatInner<S>;
