...
type CtorToReturnType<T> = T extends () => infer Result
    ? Result
    : T extends new (...params: any[]) => infer Result
        ? Result
        : any;

type PropType<T> = {
    [K in keyof T]: CtorToReturnType<ToUnion<T[K] extends { type: infer U } ? U : T[K]>>;
};
...
type Options<P, D, C, M> = {
    ...
    methods: ThisType<PropType<P> & D & ComputedReturnType<C> & M> & M;
};
