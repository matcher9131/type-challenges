...
type Store<S, G, A> = {
    id: string;
    state: () => S;
    getters: ThisType<Readonly<S> & GettersReturnType<G>> & G,
    actions: ThisType<S & GettersReturnType<G> & A> & A;
}

declare function defineStore<S, G, A>(store: Store<S, G, A>): {
    init(): void;
    reset(): true;
} & S & GettersReturnType<G> & A;
