...
type CapitalizeWordsInner<S extends string, Current extends string = ""> = S extends `${infer First}${infer Rest}`
    ? Uppercase<First> extends Lowercase<First>
        ? `${Capitalize<`${Current}${First}`>}${CapitalizeWordsInner<Rest>}`
        : CapitalizeWordsInner<Rest, `${Current}${First}`>
    : Capitalize<Current>;
type CapitalizeWords<S extends string> = CapitalizeWordsInner<S>;
