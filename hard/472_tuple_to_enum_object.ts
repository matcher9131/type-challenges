...
type EnumString<T extends readonly string[]> = {
    readonly [P in T[number] as Capitalize<P>]: P;
}
type ToIndexed<T extends readonly unknown[], C extends unknown[] = []> = T extends readonly [infer First, ...infer Rest]
    ? [[First, C["length"]], ...ToIndexed<Rest, [...C, unknown]>]
    : [];
type EnumNumber<T extends readonly string[]> = {
    readonly [P in ToIndexed<T>[number] as Capitalize<P[0]>]: P[1]; 
}
type Enum<T extends readonly string[], N extends boolean = false> = N extends true ? EnumNumber<T> : EnumString<T>;
