...
type StringToCharTuple<S extends string> = S extends `${infer First}${infer Rest}`
    ? [First, ...StringToCharTuple<Rest>]
    : [];

type IsPalindromeInner<T extends unknown[]> = T extends [infer First, ...infer Middle, infer Last]
    ? Equal<First, Last> extends true
        ? IsPalindromeInner<Middle>
        : false
    : true;

type IsPalindrome<T extends string | number> = IsPalindromeInner<StringToCharTuple<`${T}`>>;
