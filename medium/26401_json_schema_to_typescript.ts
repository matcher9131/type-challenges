...
type JSONSchema2TS<T extends JSONSchema> = "enum" extends keyof T
                        ...
                        ? "properties" extends keyof T
                            ? "required" extends keyof T
                                ? T["required"] extends string[]
                                    ? T["properties"] extends infer U extends Record<string, JSONSchema>
                                        ? FlattenObject<{
                                            [P in TupleToUnion<T["required"]>]: P extends keyof U ? JSONSchema2TS<U[P]> : never
                                        } & {
                                            [P in Exclude<keyof U, TupleToUnion<T["required"]>>]?: JSONSchema2TS<U[P]>
                                        }>
                                        : never
                                    : never
                            : T["properties"] extends infer U extends Record<string, JSONSchema>
                                ? { [P in keyof U]?: JSONSchema2TS<U[P]> }
                                : never
