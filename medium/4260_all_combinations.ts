...
//type AllCombinationsInner<S extends string, T = S> = S extends
type StringToUnion<S extends string> = S extends `${infer First}${infer Rest}`
    ? First | StringToUnion<Rest>
    : never;
type UnionToCombination<S extends string, T extends string = S> = S extends S
    ? S | UnionToCombination<Exclude<T, S>> | `${S}${UnionToCombination<Exclude<T, S>>}` | `${UnionToCombination<Exclude<T, S>>}${S}`
    : never;
