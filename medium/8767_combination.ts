...
type TupleToUnion<T extends unknown[]> = T extends [infer F, ...infer R]
    ? F | TupleToUnion<R>
    : never;
type CombinationInner<S extends string, T extends string = S> = S extends S
    ? S | CombinationInner<Exclude<T, S>> | `${S} ${CombinationInner<Exclude<T, S>>}` | `${CombinationInner<Exclude<T, S>>} ${S}`
    : never;
type Combination<T extends string[]> = CombinationInner<TupleToUnion<T>>;
