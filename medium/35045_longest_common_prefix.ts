...
type CharAt<S extends string, N extends number, C extends unknown[] = []> = S extends `${infer First}${infer Rest}`
    ? C["length"] extends N
        ? First
        : CharAt<Rest, N, [...C, unknown]>
    : undefined;
type MapCharAt<T extends string[], N extends number> = T extends [infer First extends string, ...infer Rest extends string[]]
    ? [CharAt<First, N>, ...MapCharAt<Rest, N>]
    : [];
type AllSame<T extends unknown[], U = never> = T extends [infer First, ...infer Rest]
    ? [U] extends [never]
        ? AllSame<Rest, First>
        : First extends U
            ? AllSame<Rest, U>
            : false
    : true;
type LongestCommonPrefixInner<T extends string[], C extends unknown[] = []> = MapCharAt<T, C["length"]> extends infer U extends unknown[]
    ? AllSame<U> extends true
        ? U[0] extends string
            ? `${U[0]}${LongestCommonPrefixInner<T, [...C, unknown]>}`
            : ""
        : ""
    : never;
type LongestCommonPrefix<T extends string[]> = LongestCommonPrefixInner<T>;
