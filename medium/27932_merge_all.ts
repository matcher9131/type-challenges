...
type MergeAllInner<T extends Record<string, unknown>[], Result extends Record<string, unknown> = {}>
    = T extends [infer First extends Record<string, unknown>, ...infer Rest extends Record<string, unknown>[]]
        ? MergeAllInner<Rest, {
            [P in keyof Result | keyof First]: (P extends keyof First ? First[P] : never) | (P extends keyof Result ? Result[P] : never)
        }>
        : Result;
type MergeAll<T extends Record<string, unknown>[]> = MergeAllInner<T>;
