...
type FindAllInner<T extends string, P extends string, C extends unknown[]> = T extends `${infer First}${infer Rest}`
    ? T extends `${P}${infer PRest}`
        ? [C["length"], ...FindAllInner<Rest, P, [...C, unknown]>]
        : FindAllInner<Rest, P, [...C, unknown]>
    : [];
type FindAll<T extends string, P extends string> = P extends "" ? [] : FindAllInner<T, P, []>;
